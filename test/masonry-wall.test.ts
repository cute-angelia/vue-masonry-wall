...
describe('MasonryWall', () => {
  ...
  it('can be installed', async () => {
    ...
    const items = wrapper.findAll<HTMLDivElement>('.masonry-itemx')
  ...
  it('reacts to item changes', async () => {
    ...
    const columns = wrapper.findAll<HTMLDivElement>('.masonry-column')
    expect(columns.length).toEqual(1)
    expect(wrapper.findAll<HTMLDivElement>('.masonry-itemx').length).toEqual(2)
    await wrapper
      .setProps({
        items: [1, 2, 3],
      })
      .then(flushPromises)
    expect(wrapper.findAll<HTMLDivElement>('.masonry-itemx').length).toEqual(3)
    await wrapper
      .setProps({
        items: [1],
      })
      .then(flushPromises)
    expect(wrapper.findAll<HTMLDivElement>('.masonry-itemx').length).toEqual(1)
  })
